...
export default function Popularity() {
  ...
  // }, []);
  let getMoreGames = () => {
    console.log(PopularityGames.length);
    console.log(PopularityGames.slice(51, 70));
  };
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Popularity-Games</title>
        <link rel="canonical" href="http://mysite.com/example" />
      </Helmet>
      <div className="row">
        {PopularityGames.length > 0 ? (
          PopularityGames.slice(0, 50).map((item, index) => (
      ...
      <div className="d-flex justify-content-center my-4">
        <button onClick={getMoreGames} className="btn btn-info">
          More Games
        </button>
      </div>
    </>
