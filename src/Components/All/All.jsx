...
export default function All() {
  ...
  const [gamesToShow, setGamesToShow] = useState(30); // Number of games to display

  const loadMoreGames = () => {
    if(gamesToShow <= allGames.length){
      setGamesToShow(gamesToShow + 50); // Increase gamesToShow by 30
    }else {
      // <div className="h-100 d-flex justify-content-center align-items-center overflow-hidden alert alert-danger">
  ...
  return (
    <>
      ...
      <div className="row">
        {allGames.length > 0 ? (
          allGames.slice(startIndex, gamesToShow).map((item, index) => (
        ...
        <div className="btn btn-warning" onClick={loadMoreGames}>Add Games</div>
      </div>
    </>
  );
}
